...
test("does not split a multi-byte character across chunks", async () => {
  ...
  //
  // The split offset is load-bearing. The ASCII prefix
  // '{"type":"token","text":"' is exactly 24 bytes, so the em dash occupies
  // bytes 24-26 and only an offset strictly inside that range exercises the
  // streaming decoder. Splitting anywhere in the prefix leaves both chunks
  // independently valid UTF-8, and the test then passes against a decoder
  // with no {stream: true} at all — green, and worth nothing.
  const encoder = new TextEncoder();
...
Run: `cd frontend && npm test`
Expected: PASS — 6 ndjson + 15 reducer + 3 copy = 24 tests.
...
Expected: build succeeds, 24 tests pass.
...
export default function DocumentTable({
  ...
  async function remove(id: number) {
    ...
    // identical to success is the worst possible outcome. Surface it the same
    // way DocumentUploader does, and only refresh when something actually
    // happened. ApiError carries the server's own user-facing message.
    setError(null);
    try {
...
- [ ] `cd frontend && npm test` — 24 tests pass
