...
- **All frontend work lives in `frontend/`.** Nothing in `backend/` changes in this phase. The frame contract the UI depends on is already pinned by three backend tests (§3.1); if a task feels like it needs a backend change, STOP and report instead.
...
**Files:**
- Create: `frontend/` (via `create-next-app`), `frontend/vitest.config.ts`, `frontend/lib/smoke.test.ts`, `frontend/components/DisclaimerBanner.tsx`
...
- [ ] **Step 6: Write the disclaimer banner**
...
- [ ] **Step 7: Put it in the shell with nav**
...
- [ ] **Step 8: Replace the default home page**
...
- Delete: `frontend/lib/smoke.test.ts`
...
```bash
cd frontend && rm lib/smoke.test.ts && npm test
```
