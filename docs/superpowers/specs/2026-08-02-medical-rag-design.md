...
- **Thresholds are measured, on a narrow corpus.** Phase 3 replaced the placeholders with `0.70 / 0.75`, chosen from 120 operating points over 40 labelled questions with zero false declines and recall 0.96 (`evals/eval_results.md`). The residual risk moved rather than closed: three FDA labels of one document type is a thin basis, and `tau_strong` was never binding on that corpus, so it rests on no evidence at all. Re-measure against any substantially different corpus.
- **The ground truth needed measuring too, and the first attempt was wrong.** Absence was originally scanned with literal phrases, which missed "history of liver disease" and "decreased hepatic, renal, or cardiac function" — so the manifest called an axis absent from labels that discuss it, and three near-miss questions were actually answerable. The scan now matches stems. The lesson generalises past this project: the test guarding against it re-derived absence by calling the same scanning function, so implementation and test shared the fault and no run could ever fail. A test must not validate a function using that function.
- **One question escapes both stages, and it is not fixable by tuning.** `n04` clears `tau_abstain` at 0.759 and the model answers it. Raising the threshold past it would refuse the lowest answerable question at 0.730. That is the boundary of the two-stage approach on this corpus, and it is reported rather than relabelled.
